...
test('loose dates: the formats people actually type', () => {
  ...
  // Modifiers, from a Wednesday: "this" can be today, "last" is always past.
  assert.equal(dayKey(parseLooseDate('this wednesday', ref)), '2026-03-04')
  assert.equal(dayKey(parseLooseDate('this friday', ref)), '2026-03-06')
  assert.equal(dayKey(parseLooseDate('wednesday', ref)), '2026-03-11')
  assert.equal(dayKey(parseLooseDate('next wednesday', ref)), '2026-03-11')
  assert.equal(dayKey(parseLooseDate('next friday', ref)), '2026-03-13')
  assert.equal(dayKey(parseLooseDate('last wednesday', ref)), '2026-02-25')
  assert.equal(dayKey(parseLooseDate('last monday', ref)), '2026-03-02')
